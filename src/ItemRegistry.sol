 ...
 * @author Lifeworld
 */
contract ItemRegistry is 
    IItemRegistry, 
    ItemRegistrySignatures, 
    Auth, 
    Hash, 
    Salt  
{
    ...
    function newItemsFor(
        ...
        _verifyNewItemsSig(userId, newItemInputs, signer, NEW_ITEMS_TYPEHASH, deadline, sig);
        ...
        returns (bytes32[] memory itemHashes, address[] memory pointers) 
    {
        ...
        for (uint256 i; i < newItemInputs.length; ++i) {
            ...
            for (uint256 j; j < newItemInputs[i].channels.length; ++j) {
                _unsafeAdd(sender, userId, itemHashes[i], newItemInputs[i].channels[j]);
            }          
        }    
    }         
